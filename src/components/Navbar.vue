    ...
    <div class="shadow">
        <div class="max-w-[72rem] mx-auto px-3 sm:px-10 ">
            <nav class="flex justify-between items-center py-5 relative">
                <div class="flex ">
                    <button class="w-8 sm:hidden " @click="store.toggleNav">
                        <span
                            class="w-5 h-[2px] bg-[#4cbbe5] block mb-1 transition-all duration-300 ease-in-out"
                            :class="store.openNav ? 'translate-x-[5px]' : ''"></span>
                        <span
                            class="w-5 h-[2px] bg-[#4cbbe5] block mb-1 transition-all duration-300 ease-in-out"
                            :class="store.openNav ? 'translate-x-0' : 'translate-x-[5px]'"></span>
                    </button>
                    <div>
                        <img
                ...
                <div
                    class="flex flex-col transition-all z-10 duration-500 ease-in-out sm:relative w-1/2 absolute top-[75%] rounded-tr-2xl rounded-bl-2xl left-3 sm:left-0 sm:bottom-0 bg-[#4cbbe5] text-white sm:bg-transparent sm:text-black sm:flex-row justify-between sm:w-2/5"
                    :class="store.openNav ? 'showmenu' : 'closemenu'">
                    <a
                        href="#"
